...
data "nsxt_policy_lb_monitor" "tkgi-api-monitor" {
  type         = "TCP"
  display_name = "tkgi-api-monitor"
  description  = "Monitor for tkgi api traffic" 
}

data "nsxt_policy_lb_app_profile" "tkgi_lb_tcp_application_profile" {
  type         = "TCP"
  display_name = "tkgi_lb_tcp_application_profile"
}