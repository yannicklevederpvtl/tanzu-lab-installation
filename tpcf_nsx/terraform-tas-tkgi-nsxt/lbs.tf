...
resource "nsxt_policy_lb_service" "tkgi-lb-service" {
    display_name      = "tkgi-lb-service"
    description       = "TKGI Load Balancing Service"
    connectivity_path = nsxt_policy_tier1_gateway.tkgi-deployment-t1-gw.path
    size              = "SMALL"
    enabled           = true
    error_log_level   = "ERROR"
}

resource "nsxt_policy_lb_pool" "tkgi-api-pool" {
    display_name        = "tkgi-api-pool"
    nsx_id              = "tkgi-api-pool"
    description         = "The server pool for TKGI api server"
    algorithm           = "ROUND_ROBIN"
    active_monitor_path = data.nsxt_policy_lb_monitor.tkgi-api-monitor.path
...
resource "nsxt_policy_lb_virtual_server" "tkgi-web-virtualserver" {
  display_name              = "tkgi-api-virtualserver"
  description               = "The Virtual Server for TKGI api traffic"
  enabled                   = true
  access_log_enabled        = true
  application_profile_path  = data.nsxt_policy_lb_app_profile.tkgi_lb_tcp_application_profile.path
  ip_address                = var.tkgi_lb_api_virtual_server_ip_address
  ports                     = ["8443","9021"]
  default_pool_member_ports = ["8443","9021"]
  service_path              = nsxt_policy_lb_service.tkgi-lb-service.path
  pool_path                 = nsxt_policy_lb_pool.tkgi-api-pool.path
}
